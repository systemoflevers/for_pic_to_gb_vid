...
/**
 * Propagate an single pixel's error to the rest of the image.
 * @param {number[]} error The pixel's error split into RGB.
 * @param {number} x The pixel's x-coordinate.
 * @param {number} y The Pixel's y-coordinate.
 * @param {ImageData} imageData The ImageData to adjust.
 * @returns The updated ImageData.
 */
function propagateError(error, x, y, imageData) {
  const i = y * imageData.width + x
  for (let c = 0; c < 3; ++c) {
    imageData.data[i * 4 + c] -= error[c];
    if (x !== imageData.width - 1)
      imageData.data[(i + 1) * 4 + c] += (error[c] * 7) / 16;
    if (y === imageData.height - 1) continue;
    if (x !== 0)
      imageData.data[(i + imageData.width - 1) * 4 + c] += (error[c] * 3) / 16;
    imageData.data[(i + imageData.width) * 4 + c] += (error[c] * 5) / 16;
    imageData.data[(i + imageData.width + 1) * 4 + c] += (error[c] * 1) / 16;
  }
  return imageData;
}
